...
class TestMemoryStore:
    ...
    async def test_add_and_get_history(self, store):
        ...
        assert history[0].content.startswith("사용자 요약:")
        assert history[0].content != "Hello"
        assert history[1].role == "assistant"
        assert history[1].content.startswith("응답 요약:")
    ...
    @pytest.mark.asyncio
    async def test_history_redacts_pii(self, store):
        """Test that stored history does not keep raw PII."""
        await store.add("session1", "My email is john@example.com", "Thanks, john@example.com")

        history = await store.get_history("session1")

        assert "john@example.com" not in history[0].content
        assert "john@example.com" not in history[1].content
        assert "[EMAIL_REDACTED]" in history[0].content or "[EMAIL_REDACTED]" in history[1].content
    ...
    @pytest.mark.asyncio
    async def test_persisted_session_file_redacts_raw_input(self, tmp_path):
        """Test that persisted session JSON stores summaries only."""
        persist_dir = tmp_path / "sessions"
        store = MemoryStore(config=MemoryConfig(persist_path=str(persist_dir), auto_persist=True))

        raw_user = "My email is john@example.com"
        raw_response = "I see that john@example.com is important"
        await store.add("session1", raw_user, raw_response)

        session_file = persist_dir / "session1.json"
