...
class TestPIIRedactor:
    ...
    def test_presidio_system_exit_falls_back_to_regex(self, monkeypatch, caplog):
        """A failed spaCy setup must not prevent regex redaction."""
        class FailingAnalyzerEngine:
            def __init__(self):
                raise SystemExit(1)

        fake_presidio = types.ModuleType("presidio_analyzer")
        fake_presidio.AnalyzerEngine = FailingAnalyzerEngine
        monkeypatch.setitem(sys.modules, "presidio_analyzer", fake_presidio)

        redactor = PIIRedactor(mock_mode=False, use_presidio=True)
        result = redactor.redact("Email me at test@example.com")

        assert redactor._presidio_analyzer is None
        assert redactor._presidio_anonymizer is None
        assert redactor._presidio_available is False
        assert "test@example.com" not in result.redacted_text
        assert "[EMAIL_REDACTED]" in result.redacted_text
        assert "Presidio unavailable; falling back to regex PII redaction." in caplog.text
