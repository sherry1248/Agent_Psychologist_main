...
class TestPatternLoader:
    ...
    def test_pattern_categories(self):
        ...
        valid_categories = [
            "self_harm", "self_harm_indirect", "self_harm_slang",
            "self_harm_chinese", "harm_to_others", "harm_to_others_chinese",
            "active_crisis", "active_crisis_chinese",
            "abuse_disclosure", "abuse_disclosure_chinese",
            "eating_disorder_crisis", "self_suffocation"
        ]
...
class TestSafetyGateway:
    ...
    async def test_high_risk_detection_exact_match(self, gateway):
        ...
        assert result.response.startswith("109 / 119 / 112") or result.response.startswith("109, 119, 112")

    @pytest.mark.asyncio
    async def test_moderate_risk_has_caution_stage(self, gateway):
        """Test that moderate-risk inputs are labeled as caution."""
        result = await gateway.check("I wish I didn't exist")
    ...
    async def test_result_contains_response(self, gateway):
        ...
        assert hasattr(result, "risk_stage")
