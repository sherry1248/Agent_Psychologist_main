#!/usr/bin/env python
"""
이 PDF는 표준적이지 않은 폰트 인코딩 방식을 사용하고 있어, 텍스트를 직접 복사하는 대신 OCR(EasyOCR) 기술을 사용하여 문자를 인식하고 추출합니다
"""
...
def extract_text_with_ocr(pdf_path: str, max_pages: Optional[int] = None) -> List[Tuple[int, str]]:
    ...
    try:
        ...
        print("Error: PyMuPDF가 설치되어 있지 않습니다.. Run: pip install easyocr")
        sys.exit(1)

    # Initialize OCR reader
    print("OCR 판독기를 초기화하는 중...")
    ...
    for page_num in range(total_pages):
        print(f"현재 페이지 {page_num + 1}/{total_pages}...", end=' ')
