...
Loads raw counseling data from the local data/raw folder first and
falls back to HuggingFace only when no raw files are available.
...
except Exception:  # Optional dependency; local raw preprocessing should still work.
    load_dataset = None
...
class DataPreparation:
    ...
    def load_raw_records(self) -> List[Dict]:
        """Load raw records from local raw files if they exist."""
        records: List[Dict] = []
        candidates = sorted(
            [
                p for p in self.raw_dir.iterdir()
                if p.suffix.lower() in {".json", ".jsonl", ".csv"}
                and not self._should_skip_raw_file(p)
            ]
        )
        ...
        logger.info(f"Loading raw files from {self.raw_dir}...")
        for path in candidates:
            try:
                records.extend(self._load_single_raw_file(path))
            except Exception as exc:
                logger.warning(f"Skipping raw file {path.name}: {exc}")

        self.stats["original_count"] = len(records)
        logger.info(f"Loaded {len(records)} raw records from local files")
        return records

    def _should_skip_raw_file(self, path: Path) -> bool:
        """Exclude non-wellness sample files from the generic raw scan."""
        skipped_names = {
            "counseling_sample.jsonl",
            "empathy_sample.jsonl",
        }
        return path.name in skipped_names

    def _load_single_raw_file(self, path: Path) -> List[Dict]:
        """Load a single raw file in JSON, JSONL, or CSV format."""
    ...
    def download_dataset(self) -> List[Dict]:
        ...
        if load_dataset is None:
            raise ImportError(
                "datasets is not available in this environment; provide raw files under data/raw"
            )
    ...
    def _extract_question_text(self, record: Dict) -> str:
        """Extract the canonical question text from supported raw aliases."""
    ...
    def _extract_answer_text(self, record: Dict) -> str:
        """Extract the canonical answer text from supported raw aliases."""
        return self.normalize_text(
            record.get("answerText")
    ...
    def _extract_topic(self, record: Dict) -> str:
        """Extract the canonical topic label from supported raw aliases."""
        return self.normalize_text(
            record.get("topic")
    ...
    def is_valid_record(self, record: Dict) -> bool:
        ...
        question = self._extract_question_text(record)
        answer = self._extract_answer_text(record)
...
""".format(
    ...
    def run(self) -> Dict:
        ...
        # Step 1: Load local raw data first, then fall back to HuggingFace.
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="data/raw",
        help="Directory containing local raw source files"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
