...
def _risk_stage_from_level(risk_level: str) -> str:
    """Map a technical risk level to the Korean stage labels used by the agent."""
...
class AnalysisParser:
    ...
    def parse_json(json_data: Dict[str, Any]) -> AnalysisResult:
        ...
        risk_stage = json_data.get("risk_stage") or _risk_stage_from_level(risk_level.value)
    ...
    def parse(response_text: str) -> AnalysisResult:
        ...
        for line in lines:
            ...
            # Parse risk stage label used by the Korean-facing UI
            elif line.upper().startswith("RISK_STAGE:"):
                result.risk_stage = line.split(":", 1)[1].strip() or result.risk_stage
        ...
        if not result.risk_stage:
            result.risk_stage = _risk_stage_from_level(result.risk_level.value)
        return result
