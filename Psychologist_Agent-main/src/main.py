...
from src.agent.action_checkin import ActionCheckinResult, classify_action_checkin
...
ACADEMIC_PRESSURE_ACTION_STEP = (
    "전부 하려고 하지 말고, 과제나 공부에서 가장 쉬운 부분을 딱 10분만 시작해볼까?"
)
...
SAFE_HINTS_DIR = Path(__file__).resolve().parents[1] / "data" / "derived"
RECOMMENDED_SAFE_HINTS_PATH = SAFE_HINTS_DIR / "balanced_safe_hints_recommended.jsonl"
FALLBACK_SAFE_HINTS_PATH = SAFE_HINTS_DIR / "balanced_safe_hints.jsonl"
DEFAULT_SAFE_HINTS_PATH = (
    RECOMMENDED_SAFE_HINTS_PATH
    if RECOMMENDED_SAFE_HINTS_PATH.exists()
    else FALLBACK_SAFE_HINTS_PATH
)
...
class PsychologistAgent:
    ...
    async def process_message(
        ...
        previous_followup = ""
        action_checkin = ActionCheckinResult()
        ...
        try:
            ...
            action_checkin = classify_action_checkin(
                user_input,
                has_previous_action=bool(
                    previous_small_action and getattr(previous_small_action, "action_text", "")
                ),
            )
            result["pipeline_details"]["agents"]["action_checkin"] = action_checkin.to_dict()
            if action_checkin.detected:
                await self.memory_store.update_last_action_status(
                    session_id,
                    action_checkin.status,
                )
            ...
            if action_checkin.detected:
                response_strategy = f"action_checkin_{action_checkin.status}"
            result["pipeline_details"]["agents"]["response_strategy"] = {
            ...
            if action_checkin.detected:
                followup_question = self._action_checkin_followup(action_checkin.status)
            ...
            should_plan_small_action = (
                ...
                or response_strategy != "vague_distress"
                or (
                    intent_result
            ...
            if (
                ...
                strategy_action = self._response_strategy_action(
                    response_strategy,
                    selected_cause=cause_exploration.selected_cause,
                )
                if strategy_action:
                    small_action_plan.action_text = strategy_action
                    small_action_plan.steps = [strategy_action]
                    small_action_plan.rationale_label = response_strategy
                    small_action_plan.rationale_tags = [response_strategy]
            ...
            if self._uses_template_response_pipeline():
                ...
                if self.use_gemini_generation:
                    ...
                    if gemini_text and not self._is_valid_cloud_llm_response(gemini_text):
                        ...
                        gemini_text = ""
                    if gemini_text and not self._is_valid_response_for_strategy(
                        gemini_text,
                        response_strategy,
                    ):
                        logger.warning(
                            "Gemini response incomplete for strategy=%s; using fallback",
                            response_strategy,
                        )
                        gemini_text = ""
                    if gemini_text:
                    ...
                    if ollama_text and not self._is_valid_response_for_strategy(
                        ollama_text,
                        response_strategy,
                    ):
                        logger.warning(
                            "Ollama response incomplete for strategy=%s; using fallback",
                            response_strategy,
                        )
                        ollama_text = ""
                    llm_generation_ms = self._elapsed_ms(llm_start)
                ...
                await self._store_structured_continuity(
                    ...
                    action_status_override=(
                        action_checkin.status if action_checkin.detected else ""
                    ),
                )
    ...
    async def _store_structured_continuity(
        ...
        await updater(
            ...
            action_status_override=action_status_override,
        )
    ...
    def _select_response_strategy(
        self,
        *,
        cause_exploration: Optional[CauseExplorationResult],
        intent_result: Any,
        user_input: str,
    ) -> str:
        """Select one response shape before follow-up and action composition."""
        ...
        cause_strategies = {
            "exam_assignment_pressure": "academic_pressure",
            "academic_deadline_exam_overload": "academic_pressure",
            "specific_academic_burden": "specific_academic_burden",
            "specific_academic_burden_after_relief": "specific_academic_burden",
            "academic_relief": "academic_relief",
            "manageable_academic_concern": "manageable_concern",
            "authority_criticism": "criticism_scolding",
            "sleep_maintenance": "sleep_problem",
            "worry_or_anxiety": "sleep_problem" if primary_intent == "SLEEP_PROBLEM" else "vague_distress",
            "lifestyle_rhythm": "sleep_problem",
            "physical_fatigue": "sleep_problem",
            "sadness_burden": "sadness_crying",
            "crying_urge": "sadness_crying",
            "physical_tension_chest": "sadness_crying",
        ...
        if selected_cause in cause_strategies:
            return cause_strategies[selected_cause]
        if primary_intent == "SLEEP_PROBLEM":
            return "sleep_problem"
        if self._is_study_exam_pressure(user_input):
            return "academic_pressure"
        return "vague_distress"

    def _response_strategy_action(self, strategy: str, *, selected_cause: str) -> str:
        """Return a concrete action matched to the selected response strategy."""
        action_checkin_actions = {
            "action_checkin_completed": "정리한 내용에서 가장 헷갈린 개념 하나를 골라, 정의와 예시를 한 줄씩 붙여보자.",
            "action_checkin_partial": "이미 정리한 것 중 하나만 골라, 헷갈리는 지점에 물음표 하나를 붙여보자.",
            "action_checkin_not_completed": "오늘은 더 줄여서, 하려던 행동의 첫 항목 제목만 적어보자.",
        }
        if strategy in action_checkin_actions:
            return action_checkin_actions[strategy]
        if strategy == "academic_pressure":
            if selected_cause == "academic_deadline_exam_overload":
                return ACADEMIC_OVERLOAD_ACTION_STEP
            return ACADEMIC_PRESSURE_ACTION_STEP
        if strategy == "specific_academic_burden":
            if selected_cause == "specific_academic_burden_after_relief":
                return SPECIFIC_ACADEMIC_BURDEN_ACTION_STEP
            return GENERAL_SPECIFIC_ACADEMIC_BURDEN_ACTION_STEP
        if selected_cause == "physical_tension_chest":
            return "어깨를 한번 아래로 내려놓고, 호흡을 천천히 세 번 내쉬어보세요."
        return {
            "academic_relief": ACADEMIC_RELIEF_ACTION_STEP,
            "manageable_concern": MANAGEABLE_ACADEMIC_CONCERN_ACTION_STEP,
            "criticism_scolding": "지적 내용, 말투, 평가받는 느낌을 세 칸으로 나눠 각각 한 단어씩 적어보세요.",
            "sleep_problem": "오늘 밤에는 깬 시간과 다시 잠드는 데 걸린 느낌을 한 번만 짧게 기록해보세요.",
            "sadness_crying": "지금 올라오는 감정과 그 직전에 있었던 일을 각각 한 단어로 적어보세요.",
            "anger_frustration": "가장 화난 이유를 한 문장으로 적고, 바로 대응하지 않아도 되는 부분을 따로 표시해보세요.",
            "self_blame": "‘나는 한심하다’라고 적기보다, 실제로 일어난 사실 하나와 그때 느낀 감정 하나를 나눠 적어보세요.",
            "recovery_improvement": RECOVERY_IMPROVEMENT_ACTION_STEP,
        }.get(strategy, "")

    def _action_checkin_followup(self, status: str) -> str:
        if status in {"completed", "partial"}:
            return "해본 부분 중에서 아직 가장 헷갈리거나 막히는 건 뭐였어?"
        return "그 행동을 못 하게 만든 건 분량이 컸던 걸까, 아니면 시작할 시간이 없었던 걸까?"
    ...
    def _compose_mock_response(
        ...
        primary_intent = self._enum_name(getattr(intent_result, "primary_intent", ""))
        selected_cause = getattr(cause_exploration, "selected_cause", "") or ""
        response_strategy = response_strategy or self._select_response_strategy(
            cause_exploration=cause_exploration,
            intent_result=intent_result,
            user_input=user_input,
        )
        ...
        if response_strategy == "action_checkin_completed":
            segments = [
                "직접 해봤다는 게 중요해. 작은 행동이라도 실제로 옮긴 건 분명한 진전이야 🙂",
                "이번에는 해낸 흐름을 이어가되, 범위를 조금 더 구체적으로 좁혀보자.",
            ]
        elif response_strategy == "action_checkin_partial":
            segments = [
                "전부가 아니어도 괜찮아. 조금이라도 해본 건 멈춰 있던 흐름을 움직였다는 뜻이야 🌱",
                "이미 한 부분을 기준으로 다음 한 걸음만 더 선명하게 잡아보자.",
            ]
        elif response_strategy == "action_checkin_not_completed":
            segments = [
                "못 했다고 말해줘서 고마워. 이건 네가 약해서가 아니라 행동이 조금 컸거나 타이밍이 맞지 않았을 수 있어.",
                "부담을 더 줄이면 다시 시작할 수 있으니, 이번에는 정말 작은 단계로 바꿔보자.",
            ]
        elif selected_cause == "authority_criticism":
            segments = [
            ...
            burden = self._specific_academic_burden(user_input)
            segments = [
                f"{academic_target}에서 {burden} 어디서부터 손대야 할지 막막한 것 같아.",
                "개념, 용어, 알고리즘 흐름이 한꺼번에 나오면 누구라도 정리 순서를 잡기 어려워 😥",
            ]
            ...
            segments = [
                "공부 때문에 많이 지쳐 있는 것 같아. 시험, 과제, 성적 걱정이 겹치면 누구라도 버겁게 느낄 수 있어 😥",
                "괜찮아, 이건 네가 약해서 그런 게 아니야. 공부량과 마감 압박이 한꺼번에 몰려서 시작점이 흐려진 걸 수 있어.",
            ]
        elif selected_cause == "self_blame":
            segments = [
                "스스로를 한심하다고 느낄 만큼 마음이 많이 지쳐 있는 것 같아.",
                "하지만 지금 드는 생각이 곧 당신 전체를 설명하는 건 아니야.",
            ]
        elif selected_cause == "anger_frustration":
            segments = [
                "짜증이나 화가 올라올 만큼 마음을 크게 건드린 일이 있었던 것 같아.",
                "참을성이 부족해서가 아니라 억울함과 피로, 밀리는 압박이 겹치면 화가 더 세게 올라올 수 있어.",
            ]
            ...
            segments = [
                "울고 싶은 마음이 올라올 만큼 지금 감정이 많이 벅찬 것 같아.",
                "괜찮아, 이건 네가 약해서가 아니야. 마음에 남은 일과 피로가 쌓여서 감정이 밖으로 나오려는 걸 수 있어.",
            ]
        elif selected_cause == "sadness_burden":
            segments = [
                "속상하다고 말할 만큼 마음이 무거워진 일이 있었던 것 같아.",
                "한 사건 때문일 수도 있고, 피로와 서운함이 쌓여서 평소보다 더 크게 느껴진 걸 수도 있어.",
            ]
        ...
        strategy_action_step = self._response_strategy_action(
            response_strategy,
            selected_cause=selected_cause,
        )

        if response_strategy != "vague_distress" and strategy_action_step:
            action_step = strategy_action_step
        elif empathy_only:
        ...
        if response_strategy.startswith("action_checkin_"):
            segments.append(f"다음은 {action_step}")
        elif selected_cause == "academic_deadline_exam_overload":
            segments.append(f"오늘은 {action_step}")
        elif safe_small_action and action_step == safe_small_action:
            segments.append(f"오늘은 부담을 줄여서 이렇게 해볼까? {action_step}")
        elif small_action_text:
            segments.append(f"오늘은 전부 해결하려 하지 말고 이렇게 해보자. {action_step}")
        else:
    ...
    def _build_llm_naturalization_prompt(
        ...
        *,
        ...
        if compact_context.get("selected_cause") == "self_blame":
            strategy_instruction = (
                "self_blame 응답은 감정 공감, 생각과 사실의 분리, 계기가 된 구체적 일이나 "
                "누적 상황을 묻는 질문, 사실과 감정을 나눠 적는 행동을 모두 포함하라.\n"
            )
    ...
    def _is_valid_response_for_strategy(self, response_text: str, strategy: str) -> bool:
        """Reject naturalized responses that drop required strategy elements."""
        if strategy != "self_blame":
            return True
        ...
        separates_feeling_from_fact = any(
            marker in compact
            for marker in ("생각이 곧", "당신 전체", "사실", "자기평가", "느낀 감정")
        )
        explores_trigger = "?" in compact and any(
            marker in compact
            for marker in ("구체적인 일", "무슨 일", "계기", "쌓여", "언제부터")
        )
        includes_action = any(
            marker in compact for marker in ("적어보", "써보", "나눠 적", "기록해보")
        )
        avoids_agreement = "당신은 한심" not in compact and "정말 한심" not in compact
        return (
            len(clauses) >= 3
            and separates_feeling_from_fact
            and explores_trigger
            and includes_action
            and avoids_agreement
        )
