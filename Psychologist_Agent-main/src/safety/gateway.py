...
class SafetyGateway:
    ...
    async def check(self, user_input: str) -> SafetyResult:
        ...
        if not user_input or not user_input.strip():
            return SafetyResult(is_safe=True, risk_level=RiskLevel.NONE, risk_stage="관심")

        # Fast lexical fallback keeps crisis detection deterministic even when
        # semantic embeddings are unavailable or running in mock mode.
        literal_result = self._check_literal_matches(user_input)
    ...
    def _create_high_risk_result(
        ...
        if crisis_response:
            return SafetyResult(
                ...
                risk_stage="위험",
                matched_pattern=pattern.category,
        ...
        return SafetyResult(
            ...
            matched_category=pattern.category,
            similarity_score=score,
            response="109, 119, 112로 바로 연락하세요. 한국에서는 즉시 109(자살예방), 119(응급), 112(경찰) 도움을 받을 수 있습니다.",
    ...
    def _check_literal_matches(self, user_input: str) -> Optional[SafetyResult]:
        """Check for exact or substring matches against known crisis phrases."""
        text = user_input.lower()

        # Use the loaded pattern database so local keyword matching stays in sync
        # with the JSON data files.
        all_patterns = self.patterns_db.get_all_patterns()
        for pattern in all_patterns:
            ...
            if pattern_text and pattern_text in text:
                ...
                if pattern.risk_level == RiskLevel.MODERATE:
                    return self._create_moderate_risk_result(pattern, 1.0)
                if pattern.risk_level == RiskLevel.LOW:
                    return self._create_low_risk_result(pattern, 1.0)
