...
class EmbeddingManager:
    ...
    def __init__(self, config: Optional[EmbeddingConfig] = None, mock_mode: bool = False):
        ...
        # Only use mock if explicitly requested AND env is MOCK
        # If mock_mode=False is passed explicitly, always use real embeddings
        if mock_mode:
            self.mock_mode = True
        else:
    ...
    def _load_model(self):
        ...
        try:
            ...
            logger.warning(f"Embedding model failed to load, using mock mode: {exc}")
            self._model = MockEmbeddingModel()
            self.mock_mode = True
